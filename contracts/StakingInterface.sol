...
// Reference : https://github.com/PureStake/moonbeam/blob/master/precompiles/parachain-staking/StakingInterface.sol
...
interface ParachainStaking {
    ...
    function collator_nomination_count(address collator) external view returns (uint256);
    ...
    function nominator_nomination_count(address nominator) external view returns (uint256);
    ...
    function delegation_request_is_pending(address delegator, address candidate) external view returns (bool);
    ...
    function delegator_exit_is_pending(address delegator) external view returns (bool);

    /// @dev Whether there exists a pending exit for candidate
    /// Selector: eb613b8a
    /// @param candidate the candidate for which the exit request was made
    /// @return Whether a pending request exists for such delegation
    function candidate_exit_is_pending(address candidate) external view returns (bool);
    ...
    function candidate_request_is_pending(address candidate) external view returns (bool);
